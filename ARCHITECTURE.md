...
| mmx CLI (image) | 图片生成 | storyboard/SKILL.md (Step 4.5) | skills/storyboard/image-config.json |
| mmx CLI (vision) | 截图视觉审查 | references/storyboard/REFERENCE.md | tools/audit/vision_rules.py |
| generate-images.sh | 扫描 outline 图片标记 → mmx 生成 | storyboard/SKILL.md (Step 4.5) | tools/generate-images.sh |
| capture-screenshots.sh | Puppeteer 截图 + 可选 VV 审计 | references/storyboard/REFERENCE.md | tools/capture-screenshots.sh |
...
| mmx CLI (video) | 视频补充素材 | references/storyboard/REFERENCE.md | skills/storyboard/video-config.json |
| generate-videos.sh | 扫描 outline 视频标记 → mmx 异步生成 | references/storyboard/REFERENCE.md | tools/generate-videos.sh |
...
| 规则 ID | 检测内容 | 修复动作 |
|---------|---------|---------|
| `CH-001` | 整章纯文字无视觉演示 | 添加至少 1~2 处动态元素 |
| `CH-002` | 清单/列表一个 step 全部 stagger 上来 | 逐个揭示，1 项 = 1 step |
| `CH-003` | 紫粉渐变 / 圆角彩色边框 / emoji 当图标 | 用视角 Mental Models 重新设计 |
| `CH-004` | 假数据 / 假 logo / 假"X 万用户" | 阻断，要求替换为真实数据 |
| `CH-005` | 全章同一种入场动画（全场 fade / 全场 blur） | 注入视角的视觉多样性 |
| `CH-006` | 每步都挂 ken burns / 光晕呼吸 / 持续闪烁 | 精简为 1-2 种动画 |

### 口语化反模式

| 规则 ID | 检测内容 | 修复动作 |
|---------|---------|---------|
| `DS-001` | 信息保留度 <60%（字符比不达标） | 补充关键事实/案例/论证链 |
| `DS-002` | 长句未拆（单句 >20 字，B 站基准） | 拆分为短句 |
| `DS-003` | 第三人称疏离（"用户""读者"而非"你""我们"） | 替换为第二人称 |
| `DS-004` | 无钩子开头（前 3 秒无悬念/反差/利益） | 添加开场钩子 |
| `DS-005` | 结构词堆砌（"首先/其次/最后"或"下面进入第二部分"） | 融进过渡或砍掉 |
| `DS-006` | 数字未翻译（非核心数字直接念百分比/大数字） | 转化为感受或视觉对比 |
...
| 规则 ID | 检测内容 | 修复动作 |
|---------|---------|---------|
| `VV-001` | AI 味指纹（紫粉渐变/圆角彩色边框/emoji 当图标） | 重新设计视觉风格 |
| `VV-002` | 信息密度过高（单步 >10 个视觉元素） | 精简视觉元素 |
| `VV-003` | 占位卡片未替换（检测 "image · 16:9" 文字） | 替换为真实图片 |
| `VV-004` | 总结式结尾（大号"谢谢"/进度条终点） | 用视角 Decision Heuristics 设计结尾 |
| `VV-005` | 文字过多（单步 >80 字） | 拆分或精简文字 |
...
python -m tools.audit --rule VV workspace/<pipeline-id>/  # 仅视觉审查
python -m tools.audit --json workspace/<pipeline-id>/  # JSON 输出，供 CI 使用
bash tools/capture-screenshots.sh workspace/<pipeline-id> --audit  # 截图 + VV 审计
```
