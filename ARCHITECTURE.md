...
| 规则 ID | 检测内容 | 修复动作 |
|---------|---------|---------|
| `SL-001` | 开场白千篇一律（"大家好""今天我们来聊聊"） | 用视角的 Expression DNA 重写开场 |
| `SL-002` | 过度使用结构词（"首先/其次/最后"、"第一/第二/第三"） | 替换为非线性叙事结构 |
| `SL-003` | 信息密度均匀无起伏（每段字数偏差 <10%） | 制造节奏：密集→舒缓→高潮 |
| `SL-004` | 结尾总是"总结一下"或"希望对你有帮助" | 用视角的 Decision Heuristics 设计结尾 |
| `SL-005` | 句式单一（连续 5 句都是"主谓宾"结构） | 注入视角的 Expression DNA 变化句式 |
| `SL-006` | 每段都有过渡句（"接下来""说完X我们来看看Y"） | 删除显性过渡，用内容逻辑隐性衔接 |
...
4. 每个 Skill 由独立子 Agent 执行，执行前必须重新读取对应 `SKILL.md` 和领域参考文件
