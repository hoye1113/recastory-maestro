...
description: |
  ...
  适用场景：编辑/审阅文本、口播稿润色、文章去模板化。
  检测并修复 24 种模式：夸大的象征意义、宣传性语言、肤浅分析、模糊归因、
  破折号过度使用、三段式法则、AI 词汇、否定式排比、过多连接性短语等。
allowed-tools:
  - Read
  - Write
  - Edit
...
| 场景 | 处理方式 |
|------|---------|
| 输入文本已经很自然，无 AI 痕迹 | 直接报告「文本已符合要求，未发现明显 AI 痕迹」，不强行修改 |
| 用户未指定语调/风格 | 默认保持原文语调；若原文语调混合（如半正式半口语），先询问用户目标语调 |
| 同一段落存在多种 AI 模式叠加 | 按优先级处理：宣传性语言 > 模糊归因 > AI 词汇 > 风格问题（先改影响理解的，再改影响质感的） |
| 文本过短（<50 字） | 提示用户文本太短，模式检测可能不准确，建议提供更多上下文 |
| 用户要求「只改某类问题」 | 只处理指定模式，跳过其他 |
| 改写后含义可能发生偏差 | 标注存疑段落，附上原文和改写理由，由用户决定是否保留 |
| 技术/学术文本 | 降低改写激进度，保留必要术语，只去除明显的填充词和公式化结构 |
...
| 步骤 | 输入 | 操作 | 输出 |
|------|------|------|------|
| 1. 扫描 | 用户提供的原始文本 | 逐段阅读，对照上方 24 种模式逐一检测 | 模式命中清单 |
| 2. 分类 | 模式命中清单 | 按严重程度排序（高：影响理解；中：影响质感；低：风格偏好） | 分类后的改写计划 |
| 3. **检查点** | 改写计划 | 展示给用户确认范围和优先级 | 用户确认/调整 |
| 4. 重写 | 用户确认的改写计划 | 按优先级逐段重写，保留原文含义 | 人性化文本初稿 |
| 5. 自检 | 人性化文本初稿 | 检查：(a) 大声朗读是否自然 (b) 句子结构是否多样 (c) 是否有具体细节 (d) 语气是否一致 (e) 是否使用简单结构 | 自检通过/需返工 |
| 6. 交付 | 自检通过的文本 | 输出重写文本 + 可选的变更摘要 | 最终文本 |
...
本 Skill 可与以下工具/流程配合使用：

| 工具 | 用途 | 调用方式 |
|------|------|---------|
| distill（Recastory 子流程） | 在口播稿生成后，用 humanizer-zh 去除 AI 痕迹 | `distill` 流程的步骤 3 后加载 humanizer-zh |
| 文本编辑器 | 直接对用户提供的文本进行人性化处理 | 接收用户粘贴的文本或文件路径 |
| 质量评分（本 Skill 内置） | 对改写结果进行 5 维度评分（/50） | 处理流程步骤 5 自检时使用 |

## 与其他 Skill 的关系

- **被 distill 调用**：当 distill 生成的 script.md 触发 SL-001~006 违规规则时，自动加载 humanizer-zh 进行修复
- **被 nuwa-skill 引用**：nuwa-skill 的表达 DNA 分析可作为 humanizer-zh 的风格参考
- **独立使用**：用户直接提供文本要求去除 AI 痕迹时，独立运行
