...
在步骤描述后添加 `<!-- img: 描述 -->` HTML 注释，可被 `tools/generate-images.sh` 自动识别并调用 mmx image generate 生成图片：
...
此标记与「素材清单」互不干扰：`<!-- img: -->` 是机器可读的生成触发器，「素材清单」是人类可读的资产清单。
...
- [ ] 如有 `<!-- img: -->` 标记，确认描述粒度适当（自然语言描述，非单个词）
