...
description: |
  ...
  适用场景：B站/YouTube/视频号内容、产品demo、talk演示、教学视频。
...
|------|---------|---------|--------|
| Phase 1.1 | 识别用户输入 | — | — |
| Phase 1.2 | 产出 script.md + outline.md | SCRIPT-STYLE.md + OUTLINE-FORMAT.md | 自检 → Checkpoint Plan |
| Phase 2.1 | 脚手架 | SKILL.md 本节 | 脚手架验证 |
| Phase 2.2 | 第 1 章（强制验收） | CHAPTER-CRAFT.md + theme.json | 用户验收 |
| Phase 2.3 | 第 2~N 章 | CHAPTER-CRAFT.md | 按模式决定 |
| Phase 3 | 音频合成（可选） | AUDIO.md | Checkpoint Audio |
| Phase 4 | 录屏 + 后期 | RECORDING.md | — |
...
- [ ] 01-example demo 页面可访问
- [ ] theme.json 已正确加载（选中的主题）
```

不通过 → 排查问题后重新执行脚手架，不要跳过验证。
...
| 问题 | 处理方式 |
|------|---------|
| 脚手架 `scaffold.sh` 执行失败 | 检查 Node.js 版本（≥18）、npm 权限、目标目录是否已存在 |
| `dev server` 启动失败 | 检查端口 5173 是否被占用，提示用户关闭或换端口 |
| 章节 CSS 冲突 | 确认每章使用独立 CSS 前缀（`.cd-` / `.mg-` / `.pm-`），不要共享类名 |
| `narrations.ts` 长度与 step 不一致 | 检查章节 `.tsx` 中 `if (step === N)` 的最大 N + 1 是否等于 `narrations.length` |
| 音频合成失败（mmx-cli 不可用） | 回退到 openai provider（需 OPENAI_API_KEY），或跳过 Phase 3 |
| `npx tsc --noEmit` 报错 | 修复类型错误后重新构建，不要跳过类型检查 |
| 用户验收时要求大幅修改 | 定位问题层级（节奏/视觉/内容/代码），改最小切片，不要重做整章 |
...
本 Skill 的完整流程可能跨越多轮对话。为避免 agent 遗忘关键约束：

**每章开工前必须重读**：
- `CHAPTER-CRAFT.md` 的 Part 0（十条原则）和 Part 6（代码硬规则）
- 当前章节的 outline.md 段落（含信息池）
- `article.md` 本章对应段落（双源原则）

**关键约束提醒**（放在每章 subagent prompt 开头）：
1. `narrations.ts` 是 step 数的唯一真相源
2. 每章独立 CSS 前缀
3. 禁纯文字章节（必须有视觉演示）
4. 清单/列表必须 1 项 = 1 step
5. 完工后跑 `npx tsc --noEmit`

**会话中断恢复**：如果会话中断后恢复，先检查 `presentation/src/registry/chapters.ts` 确认已完成的章节数，再继续。
