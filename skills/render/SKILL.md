...
将 storyboard 幻灯片项目和 voice 音频合成为最终 MP4 视频。流水线：合并步骤级 MP3 → 启动 Vite 开发服务器 → CDP Screencast 录屏 + FFmpeg 编码 → 拼接章节视频 → 可选 BGM 混音。所有文字均为 HTML 元素，由 CDP screencast 自然捕获，无需额外字幕烧录。
...
4. **拼接章节视频**：FFmpeg concat demuxer 拼接所有章节为 final.mp4
5. **生成 manifest.json**：pipeline_id、created_at、output 路径、duration、resolution
