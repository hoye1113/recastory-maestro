...
将 storyboard 幻灯片项目和 voice 音频合成为最终 MP4 视频。流水线：合并步骤级 MP3 → 启动 Vite 开发服务器 → FFmpeg 录屏 + Puppeteer 自动播放 → 烧入 SRT 字幕 → 拼接章节视频 → 可选 BGM 混音。
...
3. **逐章录屏**：
   - FFmpeg 录屏（按平台自动选择采集方式）：
     - Windows: `gdigrab`（`-f gdigrab -i desktop`）
     - macOS: `avfoundation`（`-f avfoundation -i "1:none"`）
     - Linux: `x11grab`（`-f x11grab -i :0.0`）
   - Puppeteer 启动 Chromium，打开 `http://127.0.0.1:<port>/?auto=1&chapter=<id>`
   - `auto=1` 参数触发自动播放：按 audio-segments.json 顺序播放每步幻灯片
   - 录制时长 = 章节音频时长 + `RECORD_BUFFER` 秒（默认 3 秒）
   - 录屏结束后自动关闭 Puppeteer 页面
4. **烧入 SRT 字幕**：将章节级 SRT 烧入录屏视频
...
| 录屏 | `render/<chapter>-raw.mp4` | 文件存在且时长 > 0 |
| 字幕烧录 | `render/<chapter>.mp4` | 文件存在且包含字幕流 |
...
| 规则 | 检测 | 修复 |
|------|------|------|
| RD-001 | MP3 文件名与 outline 步骤编号不匹配 | 重命名文件对齐 |
| RD-002 | SRT 单句 >50 字 | 拆分短句重新合成 |
| RD-003 | 语速 <120 或 >180 字/分 | 调整 TTS --speed 参数 |
| RD-004 | 章节目录下缺少合并 SRT | 运行 merge-srt.sh |
...
| ID | 名称 | 检测方式 | 严重度 |
| -- | ---- | -------- | ------ |
| RD-001 | 编号不匹配 | MP3 文件名与 outline 步骤不一致 | critical |
| RD-002 | 字幕句子过长 | SRT 单句 >50 字 | warning |
| RD-003 | 语速异常 | <120 或 >180 字/分 | warning |
| RD-004 | 缺少章节 SRT | 章节目录下无合并 SRT 文件 | warning |
