...
name: audit
version: 1.0.0
description: 运行确定性反模式规则检查（42 条规则），解读结果并指导修复。用于产出文件的质量门控，在 critique（LLM 深度审查）之前运行。触发词：/recastory audit、"检查质量"、"跑审计"。
---
...
>   规则集: TR-001~005, CD-001/002/003/005/006, VO-001~004, SL-001~006, DS-001~006, CH-001~006, SB-001~005, VV-001~005
...
| 资源 | 路径 | 用途 |
| ---- | ---- | ---- |
| 审计工具 | `tools/audit/` | 确定性规则引擎（42 条规则） |
