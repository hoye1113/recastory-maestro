...
```
Phase 0    入口分流 → 直接路径(0A) 或 诊断路径(0B)
Phase 0.5  创建 Skill 目录结构
Phase 1    多源信息采集（6个并行Agent）
Phase 1.5  [检查点] 调研质量 Review → 用户确认
Phase 2    框架提炼（心智模型/决策启发式/表达DNA/价值观）
Phase 2.5  [检查点] 提炼确认 → 用户确认
Phase 3    Skill 构建（读模板→填充→自检→输出）
Phase 3.5  [检查点] 构建自检
...
**正确做法**：一轮追问就够了。用户说「互联网产品决策」→ 直接推荐候选。
...
Phase 3 构建完成后，**必须暂停**，执行以下自检：

```
构建自检清单：
- [ ] frontmatter description 包含触发词和适用场景
- [ ] 心智模型数量 3-7 个，每个有来源证据
- [ ] 决策启发式 5-10 条，每条有场景+案例
- [ ] 表达 DNA 6 维度完整（句式/词汇/节奏/幽默/确定性/引用）
- [ ] 诚实边界 ≥3 条具体局限
- [ ] 回答工作流（Agentic Protocol）已生成
- [ ] 创建者归属已添加
- [ ] 所有引用路径可达（references/ 下的文件存在）
```

不通过的项 → 回到对应 Phase 修复后再进入质量验证。
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 用户说「造XX的skill」但没有更多信息 | Phase 0A 缺少引导 | 按 Phase 0A 的 5 个确认点逐一询问，不要假设 |
| 调研结果全是二手转述 | 搜索词不够具体 | 换用此人的一手著作名、访谈标题、具体事件名搜索 |
| 心智模型提取不出来 | 调研素材不足 | 降低模型数量要求（2-3个），在诚实边界中标注信息不足 |
| 生成的 Skill 读起来像通用鸡汤 | 表达 DNA 提取不够 | 回到 Phase 2.3，从此人的高频用词和句式中提取更具体的规则 |
| 质量验证不通过 | Phase 2 提炼方向偏差 | 回到 Phase 2，用验证失败的具体案例调整心智模型 |
| 用户提供的本地语料格式不支持 | PDF/视频/音频格式 | 用 pdf skill 读取 PDF，用 gemini-video 转写视频，音频需先转文字 |
| Skill 目录结构不完整 | Phase 0.5 执行不完整 | 检查目录清单，补全缺失的子目录和文件 |
